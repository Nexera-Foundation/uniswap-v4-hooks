...
import {OAppRead} from "@layerzerolabs/oapp-evm/contracts/interfaces/IOAppReducer.sol";
...
import {UniswapV4StatDataProvider} from "./UniswapV4StatDataProvider.sol";
...
abstract contract LZReadStatDataProvider is StatCollectorHook, OAppRead, IOAppReducer {
    ...
    uint256 private constant READ_CALLBACK_GAS_LIMIT = 300000; // Gas limit for lzRead results callback

    uint32 private immutable _eid;
    uint32 private immutable _readChannel;
    uint256 private immutable _confirmations;
     ...
     */
    constructor(uint32 eid, uint32 readChannel, uint256 confirmations) {
        _eid = eid;
    ...
    function setConfig(uint256 feeRateReadingInterval_, uint256 intermediateLiquidityPoints_) onlyOwner() {
    ...
    function _prepareFeeRateReadTimestamps() internal view returns(uint64[] memory timestamps) {
        ...
        if(intermediateLiquidityPoints==0){
            uint64 liquidityReadInterval = (nowTimestamp - startTimestamp) / intermediateLiquidityPoints;
    ...
    function lzReduce(bytes calldata _cmd, bytes[] calldata _responses) external view returns (bytes memory){
        ...
        StatData psdStart = abi.decode(_responses[0], (StatData));
        StatData psdEnd = abi.decode(_responses[_responses.length], (StatData));
        uint256 sumMidLiquidity;
        if(_responses.length > 2) for(uint256 i; i<_responses.length; i++) {
            StatData psd = abi.decode(_responses[i], (StatData));
            sumMidLiquidity += psd.liquidity;
        }
        uint256 averageLiquidity = uint128((sumLiquidity + psdStart.liquidity + psdEnd.liquidity)/ _responses.length);
        uint256 feeDelta0 = psdEnd.fee0 - psdStart.fee0;
        uint256 feeDelta1 = psdEnd.fee0 - psdStart.fee0;
        // TODO make correct calculation
        uint256 feeRate =  1e18 * (feeDelta0 + feeDelta1) / liquidity;
